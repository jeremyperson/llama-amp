"""Color themes."""
from ..constants import N_

THEMES = {
    'green': dict(name=N_('Llama Green'), chassis='#101510', panel='#1b231c', control='#303b31',
                  text='#d8e5d7', accent='#52ef34', lcd='#081007', stripe='#111d12', palette='green'),
    'silver': dict(name=N_('Classic Silver'), chassis='#272936', panel='#36394a', control='#adb3bd',
                   text='#eeeeef', accent='#8cfa65', lcd='#060b08', stripe='#171d1a', palette='classic'),
    'amber': dict(name=N_('Amber'), chassis='#191510', panel='#292219', control='#463b2d',
                  text='#f0e0c5', accent='#ffba45', lcd='#100c04', stripe='#211a10', palette='amber'),
}

