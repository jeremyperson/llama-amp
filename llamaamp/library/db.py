...
CREATE VIRTUAL TABLE IF NOT EXISTS tracks_fts USING fts5(
    title, artist, album_artist, album, genre, path,
    content='tracks', content_rowid='rowid', tokenize='unicode61 remove_diacritics 2');
CREATE TRIGGER IF NOT EXISTS tracks_ai AFTER INSERT ON tracks BEGIN
    INSERT INTO tracks_fts(rowid, title, artist, album_artist, album, genre, path)
    VALUES (new.rowid, new.title, new.artist, new.album_artist, new.album, new.genre, new.path);
END;
CREATE TRIGGER IF NOT EXISTS tracks_ad AFTER DELETE ON tracks BEGIN
    INSERT INTO tracks_fts(tracks_fts, rowid, title, artist, album_artist, album, genre, path)
    VALUES ('delete', old.rowid, old.title, old.artist, old.album_artist, old.album, old.genre, old.path);
END;
...
class LibraryDB:
    ...
    # -- folders ------------------------------------------------------------
    def folders(self):
        return [row['path'] for row in self.execute('SELECT path FROM folders ORDER BY path')]

    def add_folder(self, path):
        with self.lock, self.conn:
            self.conn.execute('INSERT OR IGNORE INTO folders(path) VALUES (?)', (os.path.abspath(path),))
    ...
    def remove_folder(self, path):
        ...
        with self.lock, self.conn:
            ...
            others = [row['path'] for row in self.conn.execute('SELECT path FROM folders')]
            doomed = [row['path'] for row in self.conn.execute('SELECT path FROM tracks')
                      if under(row['path'], path) and not any(under(row['path'], other) for other in others)]
    ...
    def upsert(self, rows):
        ...
        now = time.time()
        columns = ('path', 'mtime', 'size') + TAG_COLUMNS
        ...
        with self.lock, self.conn:
            self.conn.executemany(sql, [tuple(row.get(column) for column in columns) + (row.get('added', now),)
                                        for row in rows])
